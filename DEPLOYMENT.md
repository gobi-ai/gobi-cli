...
This builds the CLI, then regenerates each skill's `references/` directory under `skills/gobi-*/` from `--help` output. The hand-written `SKILL.md` files in each skill folder are not overwritten.
...
No deploy step needed. skills.sh auto-indexes the skill folders under `skills/` from the GitHub repo. Once pushed, it's live:
